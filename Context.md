...
3. Do not write interim SANE block inside of a chat session; only at the beginning and the end when instructed to do so.
4. Read this entire file and limit the scope of the current chat session to the **project we are currently working on**.
5. Preserve formatting (✅ milestones, [x]/[ ] checklists, etc.) exactly as defined in Context.md (this file); at the bottom of this document is a template.  
